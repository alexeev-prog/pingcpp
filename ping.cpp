...
int main(int argc, char **argv) {
  ...
  for (int i = 0; i < argc; i++) {
    ...
    if (strcmp(argv[i], "-rt") == 0 ||
        strcmp(argv[i], "--response_timeout") == 0) {
...
void ping(char *ip, int count_of_packages, int timeout, int response_timeout) {
  ...
  for (int i = 0; i < count_of_packages; i++) {
    ...
    icmpPacket.meta.echo.payload =
        0b101101010110100101; // random binary data, doesnt matter
    icmpPacket.checksum = checksum(&icmpPacket, sizeof(icmpPacket));

    long int send_flag =
        sendto(sock, &icmpPacket, sizeof(icmpPacket), 0,
               (struct sockaddr *)&in_addr, socklen_t(sizeof(in_addr)));
    ...
    if (data_length_byte == -1) {
      cout << "\033[1;31m" << "Host unreachable or response timeout."
           << "\033[0m" << "   ";
    ...
    cout << "Received " << "\033[1;35m" << data_length_byte << "\033[0m"
         << " bytes of data from " << ip << "    ";
    cout << "ICMP response type: " << "\033[1;35m"
         << unsigned(icmpResponseHeader->type) << "\033[0m" << "    ";
    cout << "ICMP response code: " << "\033[1;35m"
         << unsigned(icmpResponseHeader->code) << "\033[0m" << "    ";
    cout << "ICMP response checksum: " << "\033[1;35m"
         << icmpResponseHeader->checksum << "\033[0m" << "    ";
...
void stat() {
  cout << endl
       << "Sent: " << "\033[1;35m" << sent << "\033[0m" << "   "
       << "Received: " << "\033[1;35m" << received << "\033[0m" << "    "
       << "Loss: " << "\033[1;35m" << sent - received << " ("
       << (sent * 100) / received << "%)" << "\033[0m" << endl;
}
...
void catch_ctrl_c(int signal) {
  ...
  stat();
  cout << endl
       << "\033[1;35m" << "Socket closed. Exiting..." << "\033[0m" << endl
       << endl;
  exit(signal);
}
